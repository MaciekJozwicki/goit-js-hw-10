export const fetchCountries = (name) => {
    return fetch(`https://restcountries.com/v3.1/name/${name}?fields=name,capital,poopulation,flags,languages`)
        .then(response => {
            if (!response.ok) {
...
//     fetch(`${API_URL}/${name}?fields=name,capital,poopulation,flags,languages`)
//         .then((res) => {
//             if (res.ok) {
//                 return res.json();
//             } if (!res.ok) {
...
//         return
//         `<h2 style="font-size: 40px"><img src="${countryName.flags.svg}" alt="${countryName.flags.alt} width="30" height="30"> ${countryName.name.official}</h2>
